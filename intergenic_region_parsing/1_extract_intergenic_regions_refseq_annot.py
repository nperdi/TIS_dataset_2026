"""
Script purpose:
This script extracts candidate intergenic genomic regions from an annotation BED file.

Main steps:
1. Keep only standard chromosomes (chr1–chr22, chrX, chrY).
2. Sort annotation intervals using chromosome order from the genome size file.
3. Extend each annotation interval by 2000 bp on both sides.
4. Merge overlapping or nearby extended annotation regions.
5. Compute the complement of the merged regions to obtain non-annotated regions.
6. Filter the resulting regions by length:
   - either keep regions with length > minDistance
   - or keep regions with minDistance < length < maxDistance
7. Save intermediate files for each processing step and write the final filtered BED file.
...
Output:
- BED file with filtered intergenic regions, plus intermediate step files

Notes:
- The script creates an organized output directory structure under output/.
- Final regions are named automatically based on the selected distance thresholds.
"""
