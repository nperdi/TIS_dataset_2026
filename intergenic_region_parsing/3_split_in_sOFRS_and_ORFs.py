"""
Script purpose:
This script separates candidate intergenic ORFs into two groups based on length:
1. short ORFs (< lengthThreshold), used as baseline negative sORFs
2. longer intergenic ORFs (>= lengthThreshold)

Main steps:
1. Read BED and FASTA files containing candidate intergenic ORFs for each flank size.
2. Measure the length of each ORF from the BED coordinates.
3. Split ORFs into two categories:
   - negative_sorf: ORFs shorter than the threshold
   - long_orf: ORFs with length greater than or equal to the threshold
4. Write separate BED files for the two categories.
5. Split the corresponding FASTA records using the same order as the BED records.
6. Write separate FASTA files for short and long ORFs.
7. Print summary statistics and output file locations.

Inputs:
- BED files with candidate intergenic ORFs
- FASTA files with candidate intergenic ORF sequences

Outputs:
- BED and FASTA files for baseline negative sORFs (< threshold)
- BED and FASTA files for longer intergenic ORFs (>= threshold)

Notes:
- The FASTA split assumes that the records are in exactly the same order as in the BED file.
- The threshold is controlled by lengthThreshold (for example 300 nt).
- The process is repeated for all flank sizes listed in flankRegionList.
"""
import os
